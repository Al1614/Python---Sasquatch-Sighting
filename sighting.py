...
class Sighting:
    ...
    def create_valid_sighting(cls, sighting_dict):
        ...
        query = """INSERT INTO sightings (location, description, date, num, user_id) VALUES (%(location)s, %(description)s, %(date)s, %(num)s, %(user_id)s);"""
    ...
    def get_by_id(cls, sighting_id):
        ...
        data = {"id": sighting_id}
        query = """SELECT sightings.id, sightings.created_at, sightings.updated_at, description, location, date, num,
    ...
    def get_all(cls):
        query = """SELECT 
                    sightings.id, sightings.created_at, sightings.updated_at, description, location, date, num,
                    users.id as user_id, first_name, last_name, email, password, users.created_at as uc, users.updated_at as uu
                    FROM sightings
                    JOIN users on users.id = sightings.user_id;"""
